...
- **GitHub #17** is implemented on its branch: the hotkey and the microphone on macOS, with right Command as the default hotkey there. Its checks by hand on the Mac (its task 5.1) and the Windows run of its tests are still open.
