...
Pisum Transcribe is a push-to-talk dictation app for Windows, where it runs in the system tray, and for Macs with Apple silicon, where it runs in the menu bar. Each release carries an MSI for Windows and a `.pkg` for the Mac (`add-macos-packaging`). `docs/idea.md` describes the idea, and `docs/roadmap.md` lists the planned changes. Planning uses OpenSpec: open changes are in `openspec/changes/`, finished ones in `openspec/changes/archive/`, and specs are in `openspec/specs/`.
...
src/Pisum.Transcribe/            Avalonia tray app (WinExe), no main window, for two frameworks: net10.0-windows10.0.19041.0 (win-x64, for the WinRT toast API) and net10.0 (osx-arm64, macOS 14 or later)
  ...
  MacOS/Info.plist               The template of the app bundle's Info.plist (@VERSION@ and @MINIMUM_SYSTEM_VERSION@ filled in by the build or the publish)
  ...
  TextInsertion/                 TextInserter: clipboard paste with restore or typed input, foreground window and elevation checks, secure input (ISecureInput); the Win32 parts and SharpHookKeyboardInput in Windows/
    MacOS/                       MacClipboardService (the pasteboard through the Swift helper on its own thread, read only while macOS allows it without asking), MacKeyboardInput (CGEvent keystrokes, Command+V with the layout's key of V), MacForegroundWindowTracker (the focused window and its frame through the Accessibility API), FrontmostApplication (the window list's frontmost app, for Electron apps that macOS doesn't name as focused), MacSecureInput
  ...
  SettingsWindow/                Settings dialog and section view models, HotkeyKeyNames (the hotkey editor's key names and rule per platform), SettingsApplier (live apply), IStartupRegistration (Start with Windows, Open at login)
    Windows/                     StartupRegistration (the Run key, Task Manager's override), UserRegistry
    MacOS/                       MacLoginItem (SMAppService.mainApp through the helper)
...
.github/workflows/               ci.yml (build and test on Windows and macOS, the MSI on Windows, on every PR and push to main), release.yml (bump, tag, test on both platforms, build the MSI and the signed .pkg, publish both together)
.config/dotnet-tools.json        Local tool manifest that pins WiX (`wix`), restored by build-msi.ps1
packaging/                       bump-version.sh, windows/build-msi.ps1 with the MSI's WiX source Pisum.Transcribe.wxs and the guard assert-native-dependencies.ps1, macos/build-pkg.sh with Distribution.xml, preinstall, postinstall and the guard assert-bundle.sh, third-party/ (the notices of ONNX Runtime, .NET, Avalonia and SkiaSharp); see packaging/README.md
```
...
./packaging/macos/build-pkg.sh 1.4.0-dev.1 --identity "Pisum Transcribe Development" --leaf <sha1>   # a test .pkg into artifacts/ (macOS)
...
`Directory.Build.props` records the last released version, and a release takes its version from the tag. Both ways to release, how the MSI and the `.pkg` install, upgrade and uninstall, and the project's signing certificate are in `packaging/README.md`.
...
- **A publish** (`dotnet publish -f net10.0 -r osx-arm64`, as `packaging/macos/build-pkg.sh` runs it) assembles the release bundle in the publish folder instead, with the same target: it leaves the dev bundle in `bin/` alone, puts the license and the notices into `Contents/Resources/`, and thins universal libraries to `arm64`. The build never copies ONNX Runtime's Windows ARM64 DLLs, which its package adds to every ARM64 build.
...
- **"Open at login" registers the bundle that runs.** `SMAppService.mainApp` is the running bundle, so turning the option on in the dev bundle from `bin/` makes macOS open that dev build at every login, and a rebuild or `git clean` leaves a login item that points nowhere. Turn it off again in the dev bundle before you quit it, or remove the item under *System Settings → General → Login Items*. The Integration test only reads the status and never registers.
