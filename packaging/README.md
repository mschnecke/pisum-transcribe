...
- **Removing:** turning off "Open at login" and moving the app to the Trash, as the README describes. There is no uninstaller. macOS keeps an `SMAppService` login item registered after the app is moved to the Trash, pointing into it, and still after the Trash is emptied; it can't start anything then, but it stays in the list of background items. The receipt `io.github.mschnecke.pisum-transcribe` stays until `sudo pkgutil --forget io.github.mschnecke.pisum-transcribe`.
