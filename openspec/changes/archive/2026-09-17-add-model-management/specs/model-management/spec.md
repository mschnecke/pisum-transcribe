...
A catalog model SHALL be considered installed when its file exists in `%LOCALAPPDATA%\Pisum Transcribe\models\` and its size equals the catalog size. The check SHALL NOT recompute the hash, so startup stays fast.

#### Scenario: Model file present
- **WHEN** the model file exists with the catalog size
