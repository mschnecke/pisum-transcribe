...
The tray menu SHALL contain a **Settings…** item, and double-clicking the tray icon SHALL also open the settings window. If the window is already open, it SHALL be brought to the foreground instead of opening a second one.
...
- **WHEN** the settings window is open and the user double-clicks the tray icon
...
- **THEN** a validation message says the hotkey must include a modifier or function key
...
The model section SHALL let the user choose the backend preference Auto, Vulkan (GPU) or CPU, and SHALL show the backend currently in use or the engine status if it is not ready.

#### Scenario: Current backend displayed
- **WHEN** the engine is ready on the Vulkan backend
- **THEN** the model section shows that Vulkan is in use
