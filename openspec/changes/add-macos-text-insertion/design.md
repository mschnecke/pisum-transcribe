...
- `pisum_pasteboard_restore(name, const uint8_t* buffer, int64_t length) -> Int32`: the same format as the snapshot. The helper adds the transient and restored markers, so both ends stay in Swift.
