...
- [A user enables **Start with Windows** and then moves the extracted folder, so the `Run` entry points to a missing exe and nothing starts at sign-in] → Only until the next start: `StartupRegistration` re-points an existing `Run` entry at the running exe whenever the app starts (`StartupRegistration.cs:88`), so the first start from the new folder repairs it. The README tells users to extract to a permanent folder, and to start the app once from its new folder after moving it. Velopack's stable install path removes the case (#2). No change to `StartupRegistration`.
