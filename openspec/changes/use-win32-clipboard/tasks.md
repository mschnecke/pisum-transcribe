...
- [ ] 1.3 Make `ClipboardSnapshot` an abstract record with only `IsSensitive` (design D3):
...
- [ ] 1.4 Move `RawClipboard` from WPF and its `DllImport`s to the app's generated `PInvoke` (design D2). It uses only functions from task 1.1:
...
- [ ] 2.1 Register `Win32ClipboardService` in `TextInsertionServiceCollectionExtensions`, and delete `WpfClipboardService` with its private snapshot subclass. Verify:
...
- [ ] 3.1 Move `TestWindow` from WPF to the app's generated `PInvoke` (design D2):
