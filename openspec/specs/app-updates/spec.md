...
While "Check for updates automatically" is on, the application SHALL ask GitHub for the latest release of the repository `mschnecke/pisum-transcribe` once between 1 and 10 minutes after it starts, and then every 24 hours while it runs. The time of the first check SHALL be chosen at random within that range at each start. Drafts and releases marked as pre-releases SHALL NOT count as the latest release. While the option is off, the application SHALL NOT send the request.
...
- **THEN** the default browser opens `https://github.com/mschnecke/pisum-transcribe/releases/tag/v1.2.0`
...
- **THEN** the request is a GET to `https://api.github.com/repos/mschnecke/pisum-transcribe/releases/latest` without a body
